...
double hash_to_diff(const void *target) {
  ...
  register long double m =
      ((long double)targ[3] / exp32) + ((long double)targ[2] / exp96);
  return (double)(1. / m);
}
...
bool stratum_connect(struct stratum_ctx *sctx, const char *url) {
  ...
  sprintf(sctx->curl_url, "http%s",
          strstr(url, "s://") ? strstr(url, "s://") : strstr(url, "://"));
...
static bool stratum_parse_extranonce(struct stratum_ctx *sctx, json_t *params,
                                     int pndx) {
  const char *xnonce1;
  int xn2_size;
...
static bool stratum_notify(struct stratum_ctx *sctx, json_t *params) {
  ...
  sctx->job.coinbase_size =
      coinb1_size + sctx->xnonce1_size + sctx->xnonce2_size + coinb2_size;
  sctx->job.coinbase =
      (uchar *)realloc(sctx->job.coinbase, sctx->job.coinbase_size);
...
static bool stratum_benchdata(json_t *result,
  ...
  json_object_set_new(val, "khashes",
                      json_real((double)global_hashrate / 1000.0));
  json_object_set_new(val, "intensity", json_real(opt_priority));
  json_object_set_new(val, "throughput", json_integer(opt_n_threads));
  json_object_set_new(val, "client",
                      json_string(PACKAGE_NAME "/" PACKAGE_VERSION));
...
static bool stratum_get_stats(struct stratum_ctx *sctx, json_t *id,
                              json_t *params) {
  char *s;
  json_t *val;
  bool ret;
...
static char *format_hash(char *buf, uint8_t *hash) {
  ...
  for (int i = 0; i < 32; i += 4) {
    len += sprintf(buf + len, "%02x%02x%02x%02x ", hash[i], hash[i + 1],
                   hash[i + 2], hash[i + 3]);
  }
  return buf;
}
...
void applog_compare_hash(void *hash, void *hash_ref) {
  ...
  for (int i = 0; i < 32; i += 4) {
    ...
    len += sprintf(s + len, "%s%02x%02x%02x%02x " CL_GRY, color, hash1[i],
